//
//  LearnJFHomeViewController.swift
//  JianSanWallpaper
//
...
extension JFHomeViewController: JFSideViewDelegate {
    ...
    */
    func didTappedCleanCacheButton() {
       ...
       let cache = "\(String(format: "%.2f", CGFloat(YYImageCache.shared().diskCache.totalCost()) / 1024 / 1024))M"
        ...
        let confirmAction = UIAlertAction(title: "确定", style: UIAlertActionStyle.destructive) { (action) in JFProgressHUD.showWithStatus("正在清理")
            // 移除全部收藏
            JFFMDBManager.sharedManager.removeAllStarWallpaper()
    ...
    */
    func didTappedFeedbackButton() {
        navigationController?.pushViewController(JFFeedbackViewController(), animateed: true)
    }
